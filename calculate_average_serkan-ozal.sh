...
JAVA_OPTS="$JAVA_OPTS -XX:+UnlockExperimentalVMOptions -XX:+UseEpsilonGC -Xm128m -Xmx128m -XX:+AlwaysPreTouch"
if [[ ! "$(uname -s)" = "Darwin" ]]; then
  JAVA_OPTS="$JAVA_OPTS -XX:+UseTransparentHugePages"
fi

CONFIGS="CLOSE_STDOUT_ON_RESULT=true USE_SHARED_ARENA=true USE_SHARED_REGION=true"
